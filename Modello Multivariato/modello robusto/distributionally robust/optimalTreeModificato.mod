...
/*parametro perturbazione*/
#param ro;

/**/
#param h_xi {OSSERVAZIONI, FEATURES, 1..5};
...
/*usata per linearizzare il valroe assoluto di a*/
...
var eta_sx {OSSERVAZIONI, BRANCH_NODES} <= 1, >= -1;
var eta_dx {OSSERVAZIONI, BRANCH_NODES} <= 1, >= -1;

var beta_sx {OSSERVAZIONI, FEATURES, BRANCH_NODES} <= 1, >= 0;
var beta_dx {OSSERVAZIONI, FEATURES, BRANCH_NODES} <= 1, >= 0;

var pi_greco_sx {OSSERVAZIONI, FEATURES, BRANCH_NODES, 1..5} <= 1, >= 0;
var pi_greco_dx {OSSERVAZIONI, FEATURES, BRANCH_NODES, 1..5} <= 1, >= 0;
...
#vincoli struttura albero aprte robusta
subject to eq8_3 {i in OSSERVAZIONI, t in LEAF_NODES, a_l in A_L[t]}:
	eta_sx[i,a_l] + sum{j in FEATURES} (beta_sx[i,j,a_l]*varrho[j]) + mu <= b[a_l] + (100 + mu)*(1 - z[i,t]);
		
subject to eq8_4 {i in OSSERVAZIONI, t in LEAF_NODES, a_r in A_R[t]}:
	- eta_dx[i,a_r] - sum{j in FEATURES} (beta_dx[i,j,a_r]*varrho[j]) >= b[a_r] - 100*(1 - z[i,t]);
...
subject to eq12_2 {t in LEAF_NODES diff {T} : t mod 2 != 0}:
	l[t] =  d[a_l_max[t]];




/*vincoli parte robusta*/


subject to eq1_1_DRO {i in OSSERVAZIONI, t in LEAF_NODES, tt in A_L[t]}:
	sum{k in 1..5}( sum{j in FEATURES} h_i[i,j,k]*pi_greco_sx[i,j,tt,k] ) <= eta_sx[i,tt] + 100*(1 - z[i,t]);
	
subject to eq1_2_DRO {i in OSSERVAZIONI, t in LEAF_NODES, tt in A_R[t]}:
	sum{k in 1..5}( sum{j in FEATURES} h_i[i,j,k]*pi_greco_dx[i,j,tt,k] ) <= eta_dx[i,tt] + 100*(1 - z[i,t]);

subject to eq2_1_DRO {i in OSSERVAZIONI, j in FEATURES, t in LEAF_NODES, tt in A_L[t]}:
	sum{k in 1..5}( sum{jj in FEATURES} C_x[j,jj,k]*pi_greco_sx[i,jj,tt,k] ) >= a[j,tt] - 100*(1 - z[i,t]);

subject to eq2_2_DRO {i in OSSERVAZIONI, j in FEATURES, t in LEAF_NODES, tt in A_R[t]}:
	sum{k in 1..5}( sum{jj in FEATURES} C_x[j,jj,k]*pi_greco_dx[i,jj,tt,k] ) >= -a[j,tt] - 100*(1 - z[i,t]);

subject to eq3_1_DRO {i in OSSERVAZIONI, j in FEATURES, t in LEAF_NODES, tt in A_L[t]}:
	sum{k in 1..5}( sum{jj in FEATURES} D_x[j,jj,k]*pi_greco_sx[i,jj,tt,k] ) >= -beta_sx[i,j,tt] - 100*(1 - z[i,t]);

subject to eq3_2_DRO {i in OSSERVAZIONI, j in FEATURES, t in LEAF_NODES, tt in A_R[t]}:
	sum{k in 1..5}( sum{jj in FEATURES} D_x[j,jj,k]*pi_greco_dx[i,jj,tt,k] ) >= -beta_dx[i,j,tt] - 100*(1 - z[i,t]);
