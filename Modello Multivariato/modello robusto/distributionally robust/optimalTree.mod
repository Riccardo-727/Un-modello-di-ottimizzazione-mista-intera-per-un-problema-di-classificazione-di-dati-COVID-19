...
/*parametro usato nella funzione obiettivo per tenere sotto controllo lacomplessit� dell'albero*/
param alpha;
...
/* c(k,t)=1 se la classe del nodo t � k*/
...
var eta {OSSERVAZIONI, BRANCH_NODES} <= 10, >= -10;

var beta {OSSERVAZIONI, FEATURES, BRANCH_NODES} <= 10, >= 0;

var pi_greco {OSSERVAZIONI, FEATURES, BRANCH_NODES, 1..5} <= 10, >= 0;
...
#questi due vincoli	servono per dire che a_capp � una delle due variabili (a o -a) altrimenti avrei un intervallo di valori che pu� assumere a ovvero a_capp
...
subject to eq8_3 {i in OSSERVAZIONI, t in LEAF_NODES, a_l in A_L[t]}:
	eta[i,a_l] + sum{j in FEATURES} (beta[i,j,a_l]*varrho[j]) + mu <= b[a_l] + (20 + mu)*(1 - z[i,t]);
		
subject to eq8_4 {i in OSSERVAZIONI, t in LEAF_NODES, a_r in A_R[t]}:
	- eta[i,a_r] - sum{j in FEATURES} (beta[i,j,a_r]*varrho[j]) >= b[a_r] - 20*(1 - z[i,t]);
...
subject to eq12_2 {t in LEAF_NODES diff {3} : t mod 2 != 0}:
	l[t] =  d[a_l_max[t]];
*/
...
subject to eq1_DRO {i in OSSERVAZIONI, t in LEAF_NODES, tt in A_L[t] union A_R[t]}:
	sum{k in 1..5}( sum{j in FEATURES} h_i[i,j,k]*pi_greco[i,j,tt,k] ) <= eta[i,tt] + 100*(1 - z[i,t]);

subject to eq2_1_DRO {i in OSSERVAZIONI, j in FEATURES, t in LEAF_NODES, tt in A_L[t]}:
	sum{k in 1..5}( sum{jj in FEATURES} C_x[j,jj,k]*pi_greco[i,jj,tt,k] ) >= a[j,tt] - 100*(1 - z[i,t]);

subject to eq2_2_DRO {i in OSSERVAZIONI, j in FEATURES, t in LEAF_NODES, tt in A_R[t]}:
	sum{k in 1..5}( sum{jj in FEATURES} C_x[j,jj,k]*pi_greco[i,jj,tt,k] ) >= -a[j,tt] - 100*(1 - z[i,t]);

subject to eq3_DRO {i in OSSERVAZIONI, j in FEATURES, t in LEAF_NODES, tt in A_L[t] union A_R[t]}:
	sum{k in 1..5}( sum{jj in FEATURES} D_x[j,jj,k]*pi_greco[i,jj,tt,k] ) >= -beta[i,j,tt] - 100*(1 - z[i,t]);
