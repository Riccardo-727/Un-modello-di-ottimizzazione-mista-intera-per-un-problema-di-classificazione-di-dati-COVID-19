...
#ovvero un' osservazione x(i) puo' cadere solo un singolo nodo foglia
...
subject to eq8_1 {i in OSSERVAZIONI, t in LEAF_NODES, a_l in A_L[t]}:
	sum {j in FEATURES} (a[j,a_l]*x[i,j]) + ro*(sum {j in FEATURES} a_capp[j,a_l]*( std_x_classe1[j]*(y[i]) + std_x_classe0[j]*(1 - y[i]) )) + mu <= b[a_l] + (3 + mu)*(1 - z[i,t]);

subject to eq8_2 {i in OSSERVAZIONI, t in LEAF_NODES, a_r in A_R[t]}:
	sum {j in FEATURES} (a[j,a_r]*x[i,j]) - ro*(sum {j in FEATURES} a_capp[j,a_r]*( std_x_classe1[j]*(y[i]) + std_x_classe0[j]*(1 - y[i]) )) >= b[a_r] - 3*(1 - z[i,t]);
