...
#ovvero un' osservazione x(i) puo' cadere solo in un nodo foglia
...
subject to eq8_1 {i in OSSERVAZIONI, t in LEAF_NODES, a_l in A_L[t]}:
	sum {j in FEATURES} ( a[j,a_l]*x[i,j] )  +  ro*sqrt( sum {j in FEATURES}  a[j,a_l]*( cov_matrix_classe1[j,j]*y[i] + cov_matrix_classe0[j,j]*(1-y[i]) )*a[j,a_l] )  + mu  <= b[a_l] + (3 + mu)*(1 - z[i,t]);
	
subject to eq8_2 {i in OSSERVAZIONI, t in LEAF_NODES, a_r in A_R[t]}:
	sum {j in FEATURES} (a[j,a_r]*x[i,j]) - ro*sqrt( sum {j in FEATURES} a[j,a_r]*( cov_matrix_classe1[j,j]*y[i] + cov_matrix_classe0[j,j]*(1-y[i]) )*a[j,a_r] ) >= b[a_r] - 3*(1 - z[i,t]);
